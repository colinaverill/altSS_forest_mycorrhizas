...
#set output path.----
output.path <- 'figures/Supp._Fig._X._hysteresis_null_vs._feedback.png'
...
par(mfrow=c(1,2), oma=c(5.5,5,2,1), mar=c(0,0,0,0))
...
#Panel 1. hysteresis feedback simulation ramp up / ramp down.----
par(mar = c(0,3,0,0))
cols <- c('purple','green')
trans <- 0.3
...
#Panel 2.  hysteresis null simulation ramp up / ramp down.----
up <- up.null
down <- down.null
par(mar = c(0,3,0,0))
cols <- c('purple','green')
