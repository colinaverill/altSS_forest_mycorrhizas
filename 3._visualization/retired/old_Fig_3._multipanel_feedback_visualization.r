...
y <- d$y.feedback$plot.table
n <- d$n.feedback$plot.table
cols <- c('#00acd9','#cfe83c') #pick colors
trans <- 0.3 #set transparency

#get y limit.
y$cat <- cut(y$relEM, 10)
count.y <- table(y$cat)
n$cat <- cut(n$relEM, 10)
count.x <- table(n$cat)
...
par( mar = c(4,4,1,2))
#plot line.
hist(tab$relEM, breaks = 10, xlim = c(0,1), ylim = limy, ylab = NA, xlab = NA, main = NA, bty = 'l', col = cols[1], lty = 'blank')
#label.
...
#load data.
d <- readRDS(factorial_hysteresis_simulation_disturbance.path)
...
#arrows
arrows(x0 = 4, y0 = 480, x1 = 8, y1 = 400, length = 0.1, lwd = 1.5)
arrows(x0 = 7, y0 = 170, x1 = 3, y1 = 220, length = 0.1, lwd = 1.5)
