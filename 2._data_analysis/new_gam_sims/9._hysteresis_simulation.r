...
N.STEPS <- 80  #120 steps = 600 years, longer to make sure it runs to something "stable".
...
for(i in 1:length(ndep.ramp.range)){
  ...
  em.nul[[i]] <- makeitwork(
    forest.sim(g.mod.am = fits$n.feedback$G.mod.am,
               ...
               env.cov = env.cov, 
               myco.split = 'between_plot', split_frac = 0.8, silent = T,
...
for(i in 1:length(ndep.ramp.range)){
  ...
  am.nul[[i]] <- makeitwork(
    forest.sim(g.mod.am = fits$n.feedback$G.mod.am,
               ...
               r.mod.em = fits$n.feedback$R.mod.em,
               env.cov = env.cov, 
               myco.split = 'between_plot', split_frac = 0.2, silent = T,
               #disturb_rate = 0.0476/2,
               n.plots = N.PLOTS,
               n.cores = n.cores,
               n.step = N.STEPS)
