forest.sim <- function(g.mod.am, g.mod.em,
  ...
  for(i in 1:length(plot.list)){
    ...
    if(sum(!is.na(env.cov)) > 0){
      #check your random county.ID effects. they need to be common to all models.
  ...
  for(t in 1:n.step){
    ...
    new.plot.list <- 
      foreach(j = 1:length(plot.list)) %dopar% {
        ...
        recruits.prob.am <- exp(predict(r.mod.am, newdata=r.dat))
        recruits.prob.em <- exp(predict(r.mod.em, newdata=r.dat))
